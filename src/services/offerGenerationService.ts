...
export const generateOffers = async ({
  ...
  const commerceResult = await buildCommerceOffers({
    request: {
      ...
      breakfast_package: result.slots.breakfast_package ?? undefined,
      early_check_in: result.slots.early_check_in ?? undefined,
      late_check_out: result.slots.late_check_out ?? undefined,
